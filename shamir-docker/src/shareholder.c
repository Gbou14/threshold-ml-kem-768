 ...
 *
 * Stores its share-vector and performs partial_decrypt() on demand.
 * Calls the exact same partial_decrypt() from toy_crypto.c.
 *
 * REST API:
 *   GET  /health
 *     → 200 {"status":"alive"}
 *
 *   POST /store_shares
 *     Body: {"party_index":<i>, "x":<int>, "shares":[y0,y1,...]}
 *     → 200 {"status":"ok"}
 *
 *   POST /partial_decrypt
 *     Body: {"u":[u0,u1,...]}          ← ciphertext vector from coordinator
 *     → 200 {"x":<int>, "partial":<int>}
 *     → 409 if no shares stored yet
 *
 * Usage: ./shareholder <port>
 */
...
/* ── Party state ─────────────────────────────────────────────────────────── */
static int g_x            = 0;                  /* x-coordinate (Shamir index) */
static int g_share_vec[VECTOR_DIM];             /* share_vector[d] = y-value for dim d */
static int g_ready        = 0;                  /* 1 once store_shares received */
...
/* Simple int-array parser for JSON arrays like [1,2,3,4] */
...
/* ── Request context (accumulate POST body) ──────────────────────────────── */
...
static enum MHD_Result handler(void *cls,
    ...
    if (strcmp(method,"POST")==0 && strcmp(url,"/store_shares")==0) {
        ...
        printf("[sh%d] Stored: x=%d  share_vec=[", g_party_index, g_x);
    ...
    if (strcmp(method,"POST")==0 && strcmp(url,"/partial_decrypt")==0) {
        ...
        /* ── This is the exact same call as original main.c ── */
        int partial;
